...
% Výpočet v~této mapě je proveden jednotně pro všechny země z~per-country souborů \texttt{WID\_data\_\{CC\}.csv}: filtruje se proměnná \texttt{shwealj992} a~percentil \texttt{p99p100}, hodnota se převádí z~intervalu \([0,1]\) na~procenta násobením 100 a~pro každou zemi se bere poslední dostupný rok.
...
% \acloc{ČR} vykazuje podíl majetku horního~\SI{1}{\percent} na~hodnotě~\SI{26{,}3}{\percent}, přičemž mediánová hodnota v~rámci~\acs{EU}-27 čítá~\SI{24{,}4}{\percent}.
...
% Nižší koncentraci vykazují zejména severské země a~Benelux, v~nichž silný \acl{KV} a~rozsáhlé veřejné penzijní fondy distribuují výnosy z~kapitálu šířeji než systémy postavené na~individuálním spoření.
...
% Srovnání \aclins{ČR} a~Slovenska je zvláště ilustrativní: nižší slovenský podíl odpovídá přímějšímu vstupu zahraničních investorů s~atomizovanou vlastnickou strukturou (automobilový průmysl), zatímco v~\acloc{ČR} se některé průmyslové holdingové skupiny konsolidovaly v~rukou domácích vlastníků.
...
\input{texparts/python/eu_bohatstvi_top1_wid}
