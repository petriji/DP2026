...
Přesto jsou závěry konzistentní s~tezí práce a~predikovaný profil \acl{geo-CZ} po reformě pokrytí \ac{KV} odpovídá kombinaci charakteristik dvou současných států \ac{EU}:
\begin{itemize}
  \item nárůst čistého hodinového příjmu o~\SI{3,90}{\pps\per\hour} ($+39\,\%$) by \acl{geo-CZ} posunul na aktuální úroveň \acl{geo-IT} (\SI{13,54}{\pps\per\hour} v~roce 2024), zatímco \acl{geo-SE} (\SI{17,15}{\pps\per\hour}) zůstává citelně výše;
  \item zkrácení průměrné skutečné pracovní doby o~\SI{2,1}{\hour\per\week} by \acl{geo-CZ} přiblížilo \acl{geo-SE} (\SI{35,1}{\hour\per\week}), které kombinuje nejkratší pracovní dobu v~\ac{EU} s~produktivitou \num{107,4} bodu (EU27\,=\,100) --- hodnotou prakticky shodnou s~modelovou predikcí \num{108,3} pro \acl{geo-CZ};
  \item snížení \ac{GPG} o~$1{,}1$~pp je malé a~predikovaná hodnota \SI{17,4}{\percent} zůstává nad úrovní obou referenčních zemí (\acl{geo-IT}~\SI{5,3}{\percent}, \acl{geo-SE}~\SI{11,2}{\percent}); směr efektu je však konzistentní s~hypotézou o~standardizačním efektu \ac{KV} na mzdové tarify, samotné pokrytí však nestačí -- ukazatel je v~\acl{geo-CZ} tažen i~strukturálními faktory (odvětvová segregace, rodičovství), které \ac{KV} neřeší přímo.
\end{itemize}
Referenční profil tedy není jediná země, ale kombinace: příjmová úroveň \acl{geo-IT} při délce pracovní doby a~produktivitě odpovídající \acl{geo-SE}. Oba tyto státy jsou přitom charakterizovány vysokým pokrytím \ac{KV} (\acl{geo-SE}~\SI{87}{\percent} v~roce 2021, \acl{geo-IT}~\SI{100}{\percent}; dle ICTWSS \textit{AdjCov}), což odpovídá predikovanému cílovému pokrytí v~\acl{geo-CZ}.
