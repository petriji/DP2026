...
\acs{ICTWSS} a~\acs{LMP} objevují i~země mimo \aclacc{EU}, zatímco některé
členské státy \aclgen{EU} mají neúplné datové řady. Eurostatové datové sady
naopak zpravidla pokrývají všech 27 členských států \aclgen{EU}.\par
U~zemí s~datovou mezerou mohla být použita starší data, i~tato odchylka se uvádí 
...
v~národní měně na~hodnotu podle kupní srovnatelnosti (\acs{PPS}) probíhá dělením
...
\end{equation}
Pakliže jsou hodnoty vyjádřeny v~eurech, používá se konzistentní převod přes
...
protože roční čistý příjem samotný nepostihuje rozdíly v~délce skutečné pracovní
...
širším evropským panelem. U~kompozitních modelů (včetně ternární analýzy)
...
převádějí stejný příjmový prostor do~odvodů, čistého příjmu a~budoucího starobního
důchodu.\par
