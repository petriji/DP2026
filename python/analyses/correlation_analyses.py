...
_SCATTER_SPECS = [
    ...
    },
    {
        ...
        "caption": (
            "Pokrytí kolektivního vyjednávání (\\%, osa x) a Giniho koeficient "
            "disponibilního příjmu domácností po zdanění a transferech (osa y) "
            "pro státy EU27. "
            "Přerušovaná čára – regrese OLS."
        ),
    ...
    },
    {
        ...
        "caption": (
            ...
            "(PPS, EU27\\,=\\,100) v~\\% (osa y) pro státy EU27. "
            "Přerušovaná čára – regrese OLS."
        ),
...
save_figure_tex(
    "scatter_combined",
    caption=(
        "Korelace: pokrytí kolektivního vyjednávání a\N{NO-BREAK SPACE}vybrané veličiny trhu práce. "
        "(a)~průměrná prac. doba, (b)~HDP na obyvatele (EU27\,=\,100), "
        "(c)~Giniho koeficient, (d)~podíl čistého příjmu na HDP/ob. [\%]. "
        "Zabarvené body = sledované země. Přerušovaná čára \textendash{} regrese OLS."
    ),
    label="fig:scatter_combined",
    width=r"0.98\linewidth",
    cite_key="oecd_aias_ictwss_CBC_ERB_pct,eurostat_lfsa_ewhun2_HR_weekly,eurostat_nama_10_pc_PPS_EU27eq100,eurostat_ilc_di12_Gini,eurostat_earn_nt_net_PPS_AW100",
)
...
lines = [
    ...
    r"$r_{\text{akt.}}$ = Pearsonovo $r$ pro nejnovější společný rok).}",
    r"\label{tab:coverage_correlation_table}",
    r"\begin{tabular}{lrrrrr}",
    r"\toprule",
    r"Vztah & $n$ & Pearson $r$ & $p$ & Spearman $\rho$ & $r_{\text{akt.}}$ (rok) \\",
    r"\midrule",
]
for row in rows:
    p_text = "--" if np.isnan(row["p_pearson"]) else f"\\num{{{row['p_pearson']:.3f}}}"
    r_yr_text = f"{_fmt_r(row['r_year'])} ({row['year']})" if row["year"] else "–"
    lines.append(
        f"{row['label']} & {row['n']} & "
